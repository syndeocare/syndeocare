...
variable "api_public_base_url" {
  type    = string
  default = "https://api.staging.syndeocare.example.com"
}
...
variable "storage_public_bucket" {
  type    = string
  default = "syndeocare-staging-public-assets"
}

variable "storage_private_bucket" {
  type    = string
  default = "syndeocare-staging-private-documents"
}
...
module "event_backbone" {
  ...
  service_discovery_namespace_name = "staging.syndeocare.internal"
  nats_cpu                         = 512
  nats_memory                      = 1024
