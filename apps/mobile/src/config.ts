...
export const normalizeYemenPhoneInput = (value: string) =>
  value.replace(/\D/g, "").replace(/^967/, "").slice(0, 9);

export const validateYemenPhone = (value: string) =>
  YEMEN_PHONE_REGEX.test(normalizeYemenPhoneInput(value));

export const formatYemenPhone = (value: string) => {
  const digits = normalizeYemenPhoneInput(value);
  return `+967${digits}`;
};
